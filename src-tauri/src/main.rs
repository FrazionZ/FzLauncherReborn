...
use interface_profile_game::{InterfaceProfileGame, ProfileGame, TraitProfileGame};
        ...
        .on_navigation(move |url| {
                ...
                .starts_with(Auth::MICROSOFT_REDIRECTION_ENDPOINT)
            {
                ...
                tauri::async_runtime::spawn(async move {
                    match Auth::authenticate(&authorization_code).await {
                        Ok((minecraft_profile_session)) => {
                            if let Some(tx) = tx_clone.lock().unwrap().take() {
                                let _ = tx.send(Ok(minecraft_profile_session)); // Envoyer le résultat
                            }
                        }
                        Err(err) => {
                            // Gérez l'erreur ici
...
#[tauri::command]
fn get_profiles_game(
    state: State<'_, AppState>,
) -> Result<Vec<ProfileGame>, String> {
...
async fn launch_minecraft(
    ...
    if let Ok(Some(session)) = session {
        ...
        let mut child: Child = TokioCommand::new(runtime_dir)
            .args(java_args)
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .creation_flags(CREATE_NO_WINDOW)
...
#[tauri::command]
fn add_profile(name: String, id: String, refresh_token: String, app_handle: tauri::AppHandle) -> Result<(), String> {
    let profile = Profile { name, id: id.clone(), refresh_token };
