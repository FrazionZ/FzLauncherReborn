...
export default function UserComponent() {
    ...
    return (
        <>
            <div className="cursor-pointer flex items-center w-fit gap-4 px-6 py-2 rounded-lg" onClick={() => setShowProfile(true)} style={{ background: isConnected ? 'var(--btn-gradient)' : 'var(--fzbg-1)' }}>
                ...
                <div className="flex-1 flex justify-center">
                    <span className={`text-[16px] font-bold`} style={{ color: isConnected ? 'var(--btn-text)' : '#ffffff' }}>
                        {isConnected ? fzContext.sessionMSA?.auth?.name : "Se connecter"}
                    </span>
                </div>
            </div>
                    ...
                    <>
                        ...
                        <div className="listProfile flex gap-6 mt-6 flex-col h-full overflow-y-auto flex-1">
                            {profiles.map((profile, _) => {
                                ...
                                return (
                                    <div key={_} className="flex rounded-[4px] justify-between items-center px-6 bg-[var(--fzbg-1)] h-[64px]">
